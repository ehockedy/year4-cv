...
def pose_from_point_cloud(img1, img2):
    ...
    if True:#len(points_3d_1) > 0:
        ...
        # e = np.transpose(camera_matrix) * f * camera_matrix
        # w, u, vt = cv2.SVDecomp(e)
        # r = rotation_matrix_to_euler_angles(u * w * vt)
        # t = np.transpose(u)[1]
...
def run_odometry():
    ...
    for curr_image in images[starting_photo_index:num_photos_to_do]:
        ...
        if True: #(r is not None) and (t is not None) and not math.isnan(r[0]):
            ...
            total_angle_error += abs(-imu_ang_diff - r)
            num_images+=1
            print(-imu_ang_diff, r, abs(-imu_ang_diff - r), total_angle_error, total_angle_error/num_images)
