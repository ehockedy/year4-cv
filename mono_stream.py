...
#camera_matrix = np.array([camera_focal_length_px, 0, optical_image_centre_w], 
#                         [0, camera_focal_length_px, optical_image_centre_h],
#                         [0, 0, 1])
...
def load_gps_and_imu_data(path):
    gps_file_name = os.path.join(path, "GPS.csv");
    imu_file_name = os.path.join(path, "IMU.csv");
    gps_data = [];
    imu_data = [];
...
def get_gps_data(index, gps_data):
    latitude = float(gps_data[index]['latitude'])
    longitude = float(gps_data[index]['longitude'])
    altitude = float(gps_data[index]['altitude'])
    return latitude, longitude, altitude


def gps_to_meters(lat1, lat2, lon1, lon2):
    R = 6373.0  # radius f earth in km

    dlon = np.deg2rad(abs(lon2 - lon1))
    dlat = np.deg2rad(abs(lat2 - lat1))

    a = np.sin(dlat / 2)**2 + np.cos(lat1) * np.cos(lat2) * np.sin(dlon / 2)**2
    c = 2 * np.arctan2(np.sqrt(a), np.sqrt(1 - a))

    distance = R * c #This is in km
...
def get_imu_angular_velocity_data(index, imu_data):
    x = float(imu_data[index]['angular_velocity_x'])
    y = float(imu_data[index]['angular_velocity_y'])
    z = float(imu_data[index]['angular_velocity_z'])
    return x, y, z


def get_imu_linear_velocity_data(index, imu_data):
    x = float(imu_data[index]['linear_acceleration_x'])
    y = float(imu_data[index]['linear_acceleration_y'])
    z = float(imu_data[index]['linear_acceleration_z'])
    return x, y, z
